...
JoinEngine::JoinEngine(uint64_t* col1, uint64_t col1_sz, uint64_t* col2, uint64_t col2_sz){
    column1 = col1;
    column2 = col2;
    col1_size = col1_sz;
    col2_size = col2_sz;
...
int JoinEngine::segmentation(){
    create_and_compute_hist_array(column1);
    create_and_compute_psum_array(column1);
    create_and_compute_new_column(column1);

    create_and_compute_hist_array(column2);
    create_and_compute_psum_array(column2);
    create_and_compute_new_column(column2);

    cout << "Both relations segmentated successfully!" << endl;
    return 0;
}

int JoinEngine::create_and_compute_hist_array(uint64_t* column1){
    //to make code more readable
    int hash_value;

    //allocate relation's hist_array and initialise
    join_column1->set_hist_array(h1_num_of_buckets);
